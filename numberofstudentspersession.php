...
if ($session->logged_in) {
    ...
    if ($session->userlevel == 2 ) {
        $result2 = $database->getteacherbyreg($registration_no);
        $teacher_id = $result2['id'];
        if ($result2) {
            $result3 = $database->getteacherfromsessions($teacher_id);
            $session_id = $result3['id'];
            $category_id = $result3['category_id'];
            $course_id = $result3['course_id'];

            // $categrory_data = $database->getcategorybyid($category_id);
            // $course_data = $database->getcoursebyid($course_id);

            //  $result4 = $database->getstudentsfromstudentsfortasks($session_id);
            // $session_id = $result4['session_id'];

        } else {
            $teacher_id = null;
            $result3 = null;
            optional: echo "Teacher not found for registration_no: $registration_no";
        }
    }
} else {
    ...
    <?php if ($session->logged_in) { ?>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                function loadTable() {
                    $.ajax({
                        url: "ajax-load-studentspersession.php?id=<?php echo $_GET['id']; ?>",
                        type: "POST",
