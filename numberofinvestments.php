...
.search-bar .form-group {
    flex: 1 1 18%; /* Reduced width for a more compact form */
    margin: 3px; /* Reduced margin between inputs */
}

/* Ensure input fields take full width of the container */
...
/* Make inputs responsive: Stack on smaller screens */
...
.search-bar input {
    padding-left: 5rem; /* Adjust left padding to give more space to the icon */
}

.search-bar .form-group i {
    font-size: 1rem; /* Adjust icon size if needed */
/*    left: 0.5rem; /* Move the icon closer to the left side */*/
/*    top: 50%;*/
   ...
   <?php if ($session->logged_in) {?>
    ...
    <div class="search-bar mt-3">
        ...
        <form class="form-inline w-100 d-flex justify-content-between" id="searchForm">
            <div class="form-group mx-2 flex-fill position-relative col-12 col-md-2">
            ...
            <div class="form-group mx-2 flex-fill position-relative col-12 col-md-2">
                <i class="fas fa-calendar-alt position-absolute left-0 top-50 translate-middle-y mx-2"></i>
                <input
                    type="text"
                    id="payment_Date"
                    class="form-control form-control-sm w-100 pl-5"
                    placeholder="2024-11-16"
                    aria-label="Search unit price"
                />
            </div>
            <div class="form-group mx-2 flex-fill position-relative col-12 col-md-2">
                <i class="fas fa-credit-card position-absolute left-0 top-50 translate-middle-y mx-2"></i>
                <input
                    type="text"
                    id="amount_paid"
                    class="form-control form-control-sm w-100 pl-5"
                    placeholder="Paid Amount"
                    aria-label="Search total amount"
                />
            </div>
            <div class="form-group mx-2 flex-fill position-relative col-12 col-md-2">
                <i class="fas fa-calendar position-absolute left-0 top-50 translate-middle-y mx-2"></i>
                <input
                    type="text"
                    id="location"
                    class="form-control form-control-sm w-100 pl-5"
                    placeholder="Location"
                    aria-label="Search payment due date"
                />
            </div>
            <div class="form-group mx-2 flex-fill position-relative col-12 col-md-2">
                <i class="fas fa-dollar-sign position-absolute left-0 top-50 translate-middle-y mx-2"></i>
                <input
                    type="text"
                    id="inverstment_number_id"
                    class="form-control form-control-sm w-100 pl-5"
                    placeholder="Investment id"
                    aria-label="Search Paid amount"
                />
            </div>
        </form>
    </div>
...
<style>
 ...
 $(document).ready(function() {
    ...
    function loadTable() {
        $.ajax({
            url: "ajax-load-investments.php", // The server-side script
            type: "POST",
    ...
    $("#company_name").on("keyup", function() {
        ...
        if (searchTerm.length > 0) {
            $.ajax({
                url: "ajax-live-search-investments.php", // Optional search script
                type: "POST",
     ...
     $("#payment_Date").on("keyup", function() {
      ...
      $("#amount_paid").on("keyup", function() {
       ...
       $("#inverstment_number_id").on("keyup", function() {
        const searchTerm = $(this).val().trim();
