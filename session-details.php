...
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	$data = $database->getsessiondetails($id);
	$classid = $data['class_id'];
	$category_id = $data['category_id'];
	$course_id = $data['course_id'];
    $teacher_id = $data['teacher_id'];
	$classdata = $database->getclassbyid($classid);
...
if (!empty($course_id)) {
    ...
    // Step 1: Split IDs
    $ids = explode(",", $course_id);

    // Step 2: Clean integer array
    $ids_clean = array_map('intval', $ids);

    // Step 3: Convert back to CSV for SQL
    $ids_string = implode(",", $ids_clean);

    // Step 4: Fetch all course titles
    $sql = "SELECT course_title FROM courses WHERE id IN ($ids_string)";
    $query = mysqli_query($database->connection, $sql);

    $names = [];
    while ($row = mysqli_fetch_assoc($query)) {
        $names[] = $row['course_title'];
    }

    // Step 5: Convert array to comma-separated list
    if (!empty($names)) {
        $course_titles = implode(", ", $names);
    }
}

    $teacherdata = $database->getteacherdetails($teacher_id);
...
<?php endif; ?>
    ...
    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="section-block" id="basicform">
            <h3 class="section-title text-center">Session Details</h3>
        </div>
        ...
        <div class="col-md-12 mb-4">
	        <div class="card position-relative">
               ...
               <?php } ?>
	            ...
	            <h5 class="card-header">Session Information</h5>
	            ...
	            <div class="card-body">
	                <div class="row">
	                    ...
	                    <div class="col-md-6 mb-3">
	                        <label class="font-weight-bold">Session Title:</label>
	                        <p class="form-control"><?php echo htmlspecialchars($data['title']); ?></p>
	                    </div>

	                    <!-- Class -->
	                    <div class="col-md-6 mb-3">
	                        <label class="font-weight-bold">Class:</label>
	                        <p class="form-control"><?php echo htmlspecialchars($classdata['name']); ?></p>
	                    </div>
	                    ...
	                    <div class="col-md-6 mb-3">
	                        ...
	                        <p class="form-control"><?php echo htmlspecialchars($course_titles); ?></p>
	                    </div>
                        <?php if ($session->userlevel == 1) {?>
                        <div class="col-md-6 mb-3">
                            <label class="font-weight-bold">Teacher:</label>
                            <p class="form-control"><?php echo htmlspecialchars($teacherdata['name']); ?></p>
                        </div>
                        <?php }?>
	                    <!-- Start Time -->
	                    <div class="col-md-6 mb-3">
	                        <label class="font-weight-bold">Start Time:</label>
	                        <p class="form-control"><?php echo htmlspecialchars($data['start_time']); ?></p>
	                    </div>

	                    <!-- End Time -->
	                    <div class="col-md-6 mb-3">
	                        <label class="font-weight-bold">End Time:</label>
	                        <p class="form-control"><?php echo htmlspecialchars($data['end_time']); ?></p>
	                    </div>
                        <?php if ($session->userlevel == 1) {?>
	                    <!-- Slots -->
	                    <div class="col-md-6 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="font-weight-bold">Booked Slots:</label>
                            <p class="form-control"><?php echo htmlspecialchars($data['booked_slots']); ?></p>
                        </div>
                        <?php $available_slots = $data['slots'] - $data['booked_slots'] ?>
                        <div class="col-md-6 mb-3">
                            <label class="font-weight-bold">Available Slots:</label>
                            <p class="form-control"><?php echo htmlspecialchars($available_slots); ?></p>
                        </div>
                    <?php }?>
	                    <!-- Created At -->
	                    <div class="col-md-6 mb-3">
