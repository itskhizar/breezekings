...
$sql = "SELECT * FROM purchases
        WHERE (supplier_name LIKE '%{$search_value}%' 
            OR purchase_date LIKE '%{$search_value}%'
            OR unitprice LIKE '%{$search_value}%'
            OR totalamount LIKE '%{$search_value}%'
        	OR payment_duedate LIKE '%{$search_value}%'
            OR paidamount LIKE '%{$search_value}%') ";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Build the table row
         $output .= "<tr>
            ...
            <td>" . htmlspecialchars($row['payment_duedate']) . "</td>
